...
export async function Hoyolab(env: Env, ctx: ExecutionContext) {
    const hoyolab = await env.DB.prepare('SELECT ltoken_v2, ltmid_v2 FROM mihoyo WHERE type = "Hoyolab Star Rail"').run<signCookies>()
    hoyolab.results.map(cookie => ctx.waitUntil(signAction(cookie)))
}