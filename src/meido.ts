...
async function CheckElec(env: Env, update: Update.MessageUpdate) {
    ...
    const user = await env.DB.prepare('SELECT * from elec WHERE user_id = ?1 LIMIT 1').bind(update.message.from.id).first<User>()
    if (user === null) {
    ...
    const elec = await async function () {
        try {
            ...
            try {
                ...
                await env.DB.prepare('UPDATE elec SET cookies = ?2 WHERE user_id = ?1')
    ...
    if (elec === undefined) {
        await env.DB.prepare('UPDATE elec SET err = ?2 WHERE user_id = ?1').bind(user.user_id, 1).run()
        return meido('sendMessage', {
...
async function work(env: Env, user: User) {
    ...
    if (elec === undefined) {
        await env.DB.prepare('UPDATE elec SET err = ?2 WHERE user_id = ?1')
            .bind(user.user_id, 1)
            .run()
        return meido('sendMessage', {
...
export async function CronCheckElec(env: Env) {
    const users = await env.DB.prepare('SELECT * from elec WHERE err = 0').all<User>()
